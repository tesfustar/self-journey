...
import { Spinner, Center, Button,useToast } from "@chakra-ui/react";
...
const Navbar = ({ toggle }) => {
  ...
  const [unSubscribePhone, setUnSubscribePhone] = useState(null);
  ...
  const blogCategory = useQuery(
    ...
    async () =>
      await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}article-categories`,
        {
          headers,
        }
      ),
  ...
  const blogData = useQuery(
    ...
    async () =>
      await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}view-scholarships-and-articles`,
        {
          headers,
        }
      ),
  ...
  const unSubscribeMutation = useMutation(
    async (newData) =>
      await axios.delete(`${process.env.REACT_APP_BACKEND_URL}link-unsubscribe`,  {
        headers,
      }),
    {
      retry: false,
  ...
  const unSubscribeMutationSubmitHandler = async (values) => {
    try {
  ...
  function confirmSubscription() {
    unSubscribeMutationSubmitHandler()
  ...
  if (unSubscribeModal) {
    return (
      <Transition appear show={unSubscribeModal} as={Fragment}>
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                ...
                >
                  ...
                  <h1 className='text-gray-500 font-medium  text-lg py-2'>
              {isAmh
              ? "የራስ ጉዞን ከደንበኝነት ምዝገባ ለመውጣት እርግጠኛ ነዎት"
              : "are u sure to unsubscribe selfjourney"}
                    ...
                    <div className="flex items-center space-x-5 pt-5">
                     ...
                     {isAmh ?   'አዎ' :'Yes'}
                      ...
                      <Button bg={"red.600"} color={"white"} onClick={close}>
                      {isAmh ?   'አይ' :'Cancel'}
                      </Button>
                    </div>
                  </div>
  ...
  return (
    <>
        ...
        <div className="fixed  z-40  w-full  p-3 ">
          <div className="max-w-6xl mx-auto flex  items-center justify-between p-3 sm:px-4 bg-white/50 rounded-md border-2 border-white">
            ...
            <div className="flex items-center space-x-5 lg:space-x-10">
              ...
              <Link
                to={`/blog/scholarship/${
                  blogData?.data?.data?.data?.scholarships &&
                  blogData?.data?.data?.data?.scholarships[0]
                    ?.article_category_id
                }`}
                className="hidden md:flex font-semibold hover:text-sky-500"
              >
              ...
              <div className="hidden md:flex  flex-col  items-center justify-center group">
                <h1 className="font-semibold cursor-pointer hover:text-sky-500">
              ...
              <div className=" hidden md:flex w-full flex-grow  flex-col items-center justify-center ">
                <div
                  className={
                    showJourneys
                      ? `${style} border-b-2 border-[#00a69c]`
                      : `${style}`
                  }
                ...
                >
                  ...
                  <BiChevronDown
                    size={20}
                    color={showJourneys ? "#00a69c" : "black"}
                  />
                </div>
              </div>
              {/* bolgs */}
                  ...
                  <div className="space-y-3 ">
                      ...
                      <div className="space x-1 ">
                        ...
                        >
                          <TiInputChecked
                            size={20}
                            className={
                              !isAmh ? "text-blue-500" : "text-gray-200"
                            }
                          />
                   ...
                   hover:bg-gray-100 p-2  rounded-md"
                        ...
                        <div className="flex items-center space-x-1 cursor-pointer hover:bg-gray-100 p-2  rounded-md">
                          ...
                          <h1 onClick={logout} className="font-semibold ">
                            {" "}
                            {isAmh ? "ውጣ" : "Logout"}
                          </h1>
                        </div>
      ...
      {showJourneys && (
        <div className="absolute sticky z-50  flex-grow p-5  w-full  top-16  flex items-center justify-center">
          <div className="bg-white shadow-md w-full">
            {categoriesData?.isFetched ? (
              <div className="flex flex-col items-center space-y-4">
                <h1 className="font-semibold text-2xl pb-2">
                  DISCOVER LIFE{" "}
                  <span className="text-[#F16A37]">JOURNEYS!</span>
                </h1>
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                  {categoriesData?.data?.data?.data.map((data) => (
                    <div
                      className="flex items-start space-x-3  cursor-pointer hover:opacity-60"
                      key={data.id}
                      onClick={() => {
                        setCategory(data?.id);
                    ...
                    >
                      <img
                        src={
                          process.env.REACT_APP_BACKEND_IMAGE_URL + data?.image
                        }
                      ...
                      <div className="flex flex-col space-y-2">
                        ...
                        <p className="text-[13px] max-w-sm w-[220px]">
                          How to become a healthy good looking active person?
                        </p>
                      </div>
                    </div>
                  ))}
