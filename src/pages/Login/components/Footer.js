    ...
    <div className='bg-[#00a69c] '>
      <div className='max-w-6xl   items-start text-center justify-center gap-7 text-md
        ...
        <div className=' flex flex-col justify-start items-start space-y-2 text-gray-100' >
          <h1 className="font-bold text-gray-100">{isAmh ? 'ፈጣን ማገናኛዎች':'Quick links'}</h1>
        <h1 className='hover:underline text-sm text-gray-100'>{isAmh ? "ቤት" : "Home"}</h1>
          <h1 onClick={handleModal} className='hover:underline text-sm text-gray-100'>{isAmh ? 'ብሎጎች':'Blogs'}</h1>
          <h1 onClick={handleModal} className='hover:underline text-sm text-gray-100'> {isAmh ? "ስኮላርሺፕ" : "Scholarships"}</h1>
          
        </div>
        {/* journey */}
        <div className=' flex flex-col justify-start items-start space-y-2 text-gray-100'>
          <h1 className="font-semibold text-gray-100">{isAmh ? "ሁሉም የጉዞ" : "All Journey's"}</h1>
        {Journeys?.isFetched ? (
        ...
        <div className=' flex flex-col justify-start items-start space-y-3'>
          ...
          <div className=' flex flex-col justify-start items-start space-y-2 text-gray-100'>
          <h1 onClick={handleModal} className="text-gray-100 text-sm cursor-pointer hover:underline">{isAmh ?'አካውንቲንግ እና ፋይናንስ' : 'Accounting and Finance'}</h1>
                <h1 onClick={handleModal}  className="text-sm text-gray-100  cursor-pointer hover:underline">{isAmh? 'ምህንድስና':'Engineering'}</h1>
                <h1 onClick={handleModal}  className="text-sm text-gray-100  cursor-pointer hover:underline">{isAmh? 'ሆቴል እና መስተንግዶ':'Hotel and Hospitality'}</h1>
                <h1 onClick={handleModal}  className="text-sm text-gray-100  cursor-pointer hover:underline">{isAmh? 'ኢንፎርሜሽን ቴክኖሎጂ':'Information Technology'}</h1>
                <h1 onClick={handleModal}  className="text-sm text-gray-100  cursor-pointer hover:underline">Management</h1>
                <h1 onClick={handleModal} className="text-sm  text-gray-100 cursor-pointer hover:underline">{isAmh? 'የጤና ጥበቃ':'Health Care'}</h1>
                <h1 onClick={handleModal} className="text-sm  text-gray-100 cursor-pointer hover:underline">{isAmh? 'ሁሉም':'Browse All'}</h1>
      ...
      <div className="pb-5 max-w-6xl mx-auto border-t border-white grid grid-cols-1 sm:grid-cols-3 p-2 gap-2">
        <div className="flex  items-center justify-center  text-gray-200">
          <h1 className="font-medium text-gray-100">2022 <span className="font-bold text-gray-100">{isAmh ? "ሰልፍ ጆርኒ" : "Self Journey"}</span> <span> {isAmh ? "የተሰራው" : "Made by"}</span>Jaktech</h1>
      </div>
        <div className="flex  items-center justify-center space-x-3 text-gray-200">
          <h1 onClick={()=>setIsOpen(true)}
          className="font-bold hover:underline cursor-pointer text-gray-100">{isAmh ? "ውሎች እና ሁኔታዎች" : "Terms & Conditions"}</h1>
          <h1 
          className="font-bold hover:underline cursor-pointer text-gray-100">{isAmh ? "የግላዊነት ፖሊሲ" : "Privacy policy"}</h1>
        </div>
