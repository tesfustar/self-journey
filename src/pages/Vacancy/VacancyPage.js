...
const VacancyPage = () => {
    ...
    console.log(catId)
    const Vacancies = useQuery([`VacanciesApi`,currentPage,vacancyId],async () =>
    await axios.get(vacancyId ?  `https://hulum.et/hulum2/api/vacancy_categories/${vacancyId}?page=${currentPage}` :`https://hulum.et/hulum2/api/selfJourney-vacancy?page=${currentPage}`),
  {
  ...
  const detailsData = useQuery(
    ...
    async () =>
      await axios.get(`https://hulum.et/hulum2/api/selfJourney-vacancy/${detailsId}`),
    {
      keepPreviousData: false,
      refetchOnWindowFocus: false,
      retry: false,
...
console.log(detailsData?.data?.data)
  const VacancyCategory = useQuery(`CategoryApi`,async () =>
  await axios.get('https://hulum.et/hulum2/api/vacancy_categories'),
  {
  keepPreviousData: false,
  refetchOnWindowFocus: false,
  retry: false,
  ...
  return (
    ...
    <div className="w-full flex flex-col items-start justify-between space-y-2">
       <div className="w-full flex items-center bg-white  border-2 border-gray-400 rounded-md">
          <input type="text" name="" id="" placeholder="search a job"
           className="flex  flex-grow border-none focus:outline-none focus:border-none focus:ring-0"/>
          <FaSearch size={22} className="pr-2 text-gray-500 flex  flex-grow-0"/>
       </div>
       {/* filter */}
       <div className=" pt-2 flex flex-col items-start    space-y-1 ">
        <h1 className="font-semibold text-sm text-gray-500">{isAmh ? '': 'Filter By'}</h1>
        <select name="" id="" onChange={(e)=>setVacancyId(e.target.value)} 
         ...
         >
          {VacancyCategory.isFetched ?(
                 VacancyCategory?.data?.data?.map((item)=>(
                  <option key={item.id} value={item.id} >{isAmh ?item.name_am : item.name}</option>
                 ))
       ...
       {Vacancies.isFetched ? (
          vacancyData?.map((item)=>{
             ...
             return(
               <div key={item?.id} className="relative flex flex-col bg-white hover:border hover:border-[#00a69c]
               flex-grow hover:scale-[1.02] transition-all ease-in-out duration-300 cursor-pointer w-full p-3 pb-12 rounded-md shadow-md space-y-2 border border-gray-300">
                  ...
                  <span className='font-bold text-sm text-[#00a69c] '>New</span>
                 </div>
                 <div className='flex items-center'>
                 <span className='font-regular line-clamp-3'>{parse(isAmh ? item.description_am:item.description)}</span>
                 </div>
                   <h1 className='font-regular text-gray text-sm text-gray-400 '>Due date:{item?.due_date}</h1>
               
               
                 <div className='absolute bottom-2 flex items-start  float-left pt-6'>
                   <button onClick={()=>setdetailsId(item.id)} className='bg-[#00a69c]  p-1 px-5 font-medium text-white rounded-md hover:opacity-70'>{isAmh ?  "ዝርዝር እይታ" :'View detail'}</button>
    ...
    {/* details grid */}
    {detailsId !==null &&<div className=" h-fit z-20 top-20 flex left-10 w-full">
    {detailsData.isFetched ? (
     <div className="bg-white shadow-md rounded-md p-4">
       <h1 className='font-bold text-lg py-2'>{detailsData?.data?.data?.title}</h1>
       <div className="border-gray-300 p-2 border-y">
        <p className="xapitalize font-medium text-[#00a69c]">job summary</p>
       </div>
       <div className="p-2 py-3">
        <div className="flex items-center justify-between w-full">
          <p>Job type</p>
          <span className='font-medium'>
              {detailsData?.data?.data?.job_type == 0 &&"Cotractural" || detailsData?.data?.data?.job_type == 1 &&" Fulltime" || detailsData?.data?.data?.job_type == 2 &&" Internship" || detailsData?.data?.data?.job_type == 3 &&" Partime" || detailsData?.data?.data?.job_type == 4 && "Remotly" }</span>
        </div>
        <div>
          <p>description</p>
          <span>{parse(isAmh ? detailsData?.data?.data?.description_am : detailsData?.data?.data?.description)}</span>
        </div>
       </div>
     </div>
    ):(
